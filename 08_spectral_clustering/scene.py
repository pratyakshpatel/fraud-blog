...
class SpectralClustering(Scene):
    def construct(self):
        ...
        self.play(Transform(caption, laplacian_caption), run_time=0.5)
        self.wait(0.4)

        # Move graph to side
        graph_group = VGroup(edge_lines, all_nodes)
        self.play(graph_group.animate.scale(0.65).move_to(LEFT * 4.0 + UP * 0.8), run_time=0.5)
        ...
        self.play(FadeIn(btags), *[GrowFromEdge(b, DOWN) for b in bars], lag_ratio=0.1, run_time=0.5)
        self.wait(0.4)
        ...
        self.play(Transform(caption, eig_caption), run_time=0.5)
        ...
        self.play(Create(bracket), FadeIn(btext), run_time=0.5)
        ...
        self.play(Create(gap_arrow), FadeIn(gap_text), run_time=0.5)
        self.wait(0.4)

        # A quick Fiedler-vector pass: one cluster separates from the rest.
        fiedler_caption = Text(
            "Fiedler vector: similar values inside each ring",
            font_size=18, color=SOFT,
        ).to_edge(DOWN, buff=0.4)
        fiedler_colors = [LEGIT, ACCENT, FRAUD]
        fiedler_anims = []
        for ci in range(3):
            for j in range(n_per):
                idx = ci * n_per + j
                fiedler_anims.append(
                    node_dots[idx].animate.set_color(fiedler_colors[ci]).set_fill(fiedler_colors[ci])
                )
        self.play(Transform(caption, fiedler_caption), *fiedler_anims, run_time=0.5)
        self.wait(0.4)
        ...
        self.play(*fly_anims, run_time=0.5)
        self.wait(0.4)
        ...
        self.play(Transform(caption, kmeans_caption), run_time=0.5)
        ...
        for ci in range(3):
            ...
            self.play(Create(cluster_circle), run_time=0.34)
        ...
        self.play(Transform(caption, reveal_caption), run_time=0.5)
        ...
        self.play(*original_color_anims, run_time=0.5)
        self.wait(0.4)
