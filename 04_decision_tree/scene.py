...
class DecisionTree(Scene):
    def construct(self):
        ...
        self.play(FadeIn(root), FadeIn(caption), run_time=0.5)
        self.wait(0.4)
        ...
        self.play(Transform(caption, build_caption), run_time=0.5)
        ...
        self.play(Create(edge_no), Create(edge_yes), FadeIn(no_lbl), FadeIn(yes_lbl), run_time=0.5)
        self.play(FadeIn(left1), FadeIn(right1), run_time=0.5)
        self.wait(0.4)
        ...
        self.play(Create(edge_n2), Create(edge_y2), FadeIn(n2_lbl), FadeIn(y2_lbl), run_time=0.5)
        self.play(FadeIn(leaf_med), FadeIn(leaf_high), run_time=0.5)
        ...
        self.play(Transform(caption, classify_caption), run_time=0.5)

        user_panel = Text("User: $8k, age 3d", font_size=14, color=ACCENT).move_to(LEFT * 5.5 + UP * 1.5)
        self.play(FadeIn(user_panel), run_time=0.5)
        self.wait(0.4)

        # User dot
        user_dot = Circle(radius=0.18, color=ACCENT, fill_color=ACCENT, fill_opacity=0.9).move_to(root.get_center() + UP * 1.0)
        self.play(FadeIn(user_dot), run_time=0.5)

        # Move to root
        self.play(user_dot.animate.move_to(root.get_center()), run_time=0.5)
        self.wait(0.4)

        # Highlight YES edge
        edge_yes_highlight = edge_yes.copy().set_stroke(ACCENT, width=6)
        self.play(Create(edge_yes_highlight), run_time=0.5)

        # Move to right1
        self.play(user_dot.animate.move_to(right1.get_center()), run_time=0.5)
        self.wait(0.4)

        # Highlight YES edge
        edge_y2_highlight = edge_y2.copy().set_stroke(ACCENT, width=6)
        self.play(Create(edge_y2_highlight), run_time=0.5)

        # Move to HIGH RISK leaf
        self.play(user_dot.animate.move_to(leaf_high.get_center()), run_time=0.5)
        ...
        self.play(Transform(caption, interp_caption), run_time=0.5)

        # Show rule path as text
        rule_box = VGroup(
            Text("deposit > $5,000", font_size=12, color=WHITE),
            Text("account age < 7 days", font_size=12, color=WHITE),
        ...
        self.play(FadeIn(rule_box), Create(rule_frame), run_time=0.5)
        self.wait(0.4)
