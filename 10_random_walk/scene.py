...
class RandomWalk(Scene):
    def construct(self):
        ...
        self.play(
            ...
            FadeIn(all_nodes), FadeIn(caption),
            run_time=0.5
        )
        self.wait(0.4)
        ...
        def simulate_walk(start, steps, walker_color, walker_id):
            ...
            self.play(FadeOut(walker_grp), run_time=0.34)
            return cur
        ...
        self.play(Transform(caption, walk_caption), run_time=0.5)
        self.wait(0.4)

        # Start 4 walkers from different fraud nodes
        starts = [0, 1, 2]
        for i, start in enumerate(starts):
            simulate_walk(start, 4, walker_colors[i], i + 1)
        ...
        self.play(Transform(caption, heat_caption), run_time=0.5)
        ...
        self.play(*heat_anims, run_time=0.5)
        self.wait(0.4)

        restart_caption = Text(
            "Higher restart keeps probability near the fraud seeds",
            font_size=18, color=SOFT,
        ).to_edge(DOWN, buff=0.4)
        local_halos = VGroup(*[
            Circle(radius=0.42, color=GREEN, stroke_width=2).move_to(npos[i])
            for i in starts
        ])
        self.play(Transform(caption, restart_caption), Create(local_halos), run_time=0.5)
        self.play(FadeOut(local_halos), run_time=0.5)
        ...
        self.play(Transform(caption, n2v_caption), run_time=0.5)
        ...
        bfs_path = [0, 1, 0, 2]  # Stays close to start
        for nxt in bfs_path:
            self.play(bfs_walker.animate.move_to(npos[nxt]), run_time=0.3)

        self.play(FadeOut(bfs_walker), run_time=0.34)
        self.wait(0.4)
        ...
        dfs_path = [0, 1, 4, 5]  # Explores outward
        for nxt in dfs_path:
            self.play(dfs_walker.animate.move_to(npos[nxt]), run_time=0.3)

        self.play(FadeOut(dfs_walker), run_time=0.34)
        self.wait(0.4)
